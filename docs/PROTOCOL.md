...
| `POST /channels/{id}/members` · `DELETE /channels/{id}/members/{uid}` | add a member (admin or channel owner) · leave (`uid` = yourself) or remove someone → 204. Anyone may leave; a channel owner or a global admin removes others; only a global admin removes an owner; the last owner can't go: `409 channel.last_owner` (delete the channel instead); a DM can't be left: `422 channel.dm`; not permitted: `403 authz.forbidden`; not a member (or no such channel): `404`. Live: the removed user gets `channel.delete {id, seq}`, the rest `channel.update`; `/sync` shows `removed_channels` and `left_members`; any call they were in ends for them (`call.ended`). Removal isn't a ban: someone removed from a **public** channel can rejoin it (`POST /channels/{id}/join`). Every channel object (`GET /channels`, `channel.update`, `/sync`) lists `members[]` with `role` (`owner` or `member`), so clients know who may remove and when a leave would be the last owner's |
...
| `POST /channels/{id}/messages` | **send a message (the only send path)**; server persists then fans out via WS. A message needs `body` text or at least one of `attachments` (up to 10 committed file ids, kept in the order given); files with no caption are fine (edits follow the same rule: a caption can be removed from a message with files, but a text-only message can't be edited to blank). Optional `client_id` (UUID, per message): a resend with one the author already stored returns that message, **200** and unchanged (even with a different body), never a duplicate; a deleted one comes back as its tombstone (`deleted_at` set, empty body). Reusing a `client_id` in **another** channel is `409 conflict`. `client_id` is echoed in the response and in `message.new`. **Mentions** (`@handle`, `@channel`/`@here`) are resolved once, at send, against the channel's members then, and **stored**: every read of the message (the POST answer, `message.new`, history, `/sync`, a resend, `message.update`) carries `mentions: [user ids]` and `mention_everyone`, so a mention received while offline still highlights. Edits don't re-resolve them; a deleted message mentions nobody; messages sent before stored mentions (2026-09-26) have none. `GET /channels` carries `unread_mentions` per channel (unread messages that mention you, not your own); an offline cache counts the same from its stored rows. Optional `reply_to_id` quotes a live message in the same channel (the message's `reply_to` excerpt carries `body` (first 140 characters, `"(deleted)"` once the target is deleted), `deleted: bool` and `attachments: int`, the target's file count: use those, not the text); a missing, deleted or other-channel target is `422 message.reply_target_gone` (offer to send without the quote), except on a resend of an already-accepted `client_id`, which returns the stored reply |
| `POST /channels/{id}/owner-offers {handle}` · `DELETE /channels/{id}/owner-offers/{uid}` · `POST /channels/{id}/owner-offers/accept` · `…/decline` | **offer ownership** to a member (an owner or a global admin) → `201 ChannelOut` (`200` if already offered: one pending offer per member) · withdraw it (owner or admin) → 204 · the recipient **accepts** → `200 ChannelOut` (they become an owner; whoever offered stays one) or **declines** → 204. An offer never expires; it waits until answered or withdrawn, and ends if either side leaves the channel. It rides on the channel object as `owner_offers: [{user_id, offered_by, created_at}]`, so an offline member gets it through `/sync`, and members online get `channel.update` (the public channel listing, for non-members, never shows offers). Only the recipient can accept or decline: anyone else gets `404 offer.not_found`. Clients highlight a channel with an offer for you and ask Accept or Decline when it's opened. Errors: `403 authz.forbidden`, `404` (not a member / `offer.not_found`), `409 channel.already_owner`, `422 channel.not_member` / `channel.dm` |
...
- Every grace use and every reuse verdict is recorded in the account's security
  ...
  is never rate-limited, so a sign-out always reaches the server.

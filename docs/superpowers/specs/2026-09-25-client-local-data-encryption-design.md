...
pub trait KeySlot: Send + Sync {
    /// The key stored under `slot`, None if there is none (then core makes one).
    fn load(&self, slot: String) -> Result<Option<Vec<u8>>, KeySlotError>;
    /// Create-only: if `slot` already holds a key (another process won), return
    /// `Exists` and core loads that one. Never overwrites.
    fn store(&self, slot: String, key: Vec<u8>) -> Result<(), KeySlotError>;
    /// Destroy `slot`'s key (crypto-erase of whatever it encrypts).
...
- **Named slots, random keys:** each local store has its own random 256-bit key in its
  own slot (e.g. `cache:<store id>`), generated by core with `getrandom`. No key is
  derived from another, so destroying one slot is a real crypto-erase of that store,
  even of copies and snapshots of its file. Core zeroizes key material after use.
  On Apple a slot is one Keychain item (the §4 attributes, with `kSecAttrAccount` =
  the slot name); on Linux, one Secret Service or portal-keyring item.

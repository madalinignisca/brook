// The macOS slice of the WebRTC framework (stasel M153) compiles `RTCAudioDevice` and the
// module behind it (ObjCAudioDeviceModule, verified in the binary) but ships the header only in
// its iOS slices. This re-exports the upstream declaration, unchanged, for macOS so Swift can
// implement a custom audio device (synthetic sources in tests; Apple voice processing later).
// On iOS the framework's own header is used. Remove once the macOS slice ships it.
